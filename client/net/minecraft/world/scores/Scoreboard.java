...
public class Scoreboard {
   ...
   protected List<Scoreboard.PackedScore> packPlayerScores() {
      return this.playerScores.entrySet().stream().flatMap((var0) -> {
         ...
         return ((PlayerScores)var0.getValue()).listRawScores().entrySet().stream().map((var1x) -> {
            return new Scoreboard.PackedScore(var1, ((Objective)var1x.getKey()).getName(), ((Score)var1x.getValue()).pack());
         });
      }).toList();
   }
   ...
   public static record PackedScore(String owner, String objective, Score.Packed score) {
      ...
      public static final Codec<Scoreboard.PackedScore> CODEC = RecordCodecBuilder.create((var0) -> {
         return var0.group(Codec.STRING.fieldOf("Name").forGetter(Scoreboard.PackedScore::owner), Codec.STRING.fieldOf("Objective").forGetter(Scoreboard.PackedScore::objective), Score.Packed.MAP_CODEC.forGetter(Scoreboard.PackedScore::score)).apply(var0, Scoreboard.PackedScore::new);
      });

      public PackedScore(String param1, String param2, Score.Packed param3) {
         super();
         this.owner = var1;
         this.objective = var2;
         this.score = var3;
